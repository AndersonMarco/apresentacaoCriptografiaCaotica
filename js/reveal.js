...
var Reveal = (function(){
	...
	function startEmbeddedContent( slide ) {
        if(indexh==1 && indexv==0){
            $(".unicoPlay").removeClass("toquei");
            $(".unicoPlay").addClass("toqueiNao");
        }
        $(".unicoPlay").each(function(index){
            var v=$(this)[0];
	 ...
	 * the targeted slide.
	 */
	function startEmbeddedContentFragment( slide ) {

		if( slide ) {
			// HTML5 media elements
			toArray( slide.querySelectorAll( 'video, audio' ) ).forEach( function( el ) {
				if( el.hasAttribute( 'data-autoplay' ) ) { 
                        if($("#"+el.id).hasClass("unicoPlay") && $("#"+el.id).hasClass("toquei")==false && $("#"+el.id).hasClass("toqueiNao")){                 			
					        $("#"+el.id).addClass("toquei");
                            $("#"+el.id).removeClass("toqueiNao");
                            el.play();
                        ...
                        if($("#"+el.id).hasClass("unicoPlay")==false){
                            el.play();
                        }
		...
		if( currentSlide && config.fragments ) {
			...
			if( fragments.length ) {
				...
				toArray( fragments ).forEach( function( element ) {
                    ...
                    startEmbeddedContentFragment( currentSlide );
				} );
